...
SPEC_TYPES = ("hex", "reference_image", "word", "resolved_word", "unchanged")
    ...
    for row in specs:
        ...
        if entry is None:
            entry = {
                ...
                # Truthy when this value is the finish the base photograph already
                # shows. Accepts anything a person would type in a spreadsheet cell.
                "unchanged": str(row.get("unchanged") or "").strip().lower() in (
                    "1", "y", "yes", "true", "t", "x", "original", "base", "as-is"),
                "refs": [],
            }
            index[key] = entry
...
def spec_type_of(entry) -> str:
    ...
    if entry.get("unchanged"):
        # Checked FIRST, and deliberately so: this value may also carry a hex, because
        # knowing the colour of the original finish is useful for the board swatch and
        # for verification even though nothing is being regenerated.
        return "unchanged"
    if entry.get("refs"):
...
def validate_intake(variants, specs, product, plates=None) -> list:
    ...
    for (axis, value), entry in sorted(index.items()):
        # ...unless the value IS the photograph. Nothing is being generated for it, so
        # there is nothing to specify: the plate already shows the answer. Without this
        # the rule fires on exactly the one value that cannot possibly be wrong.
        if entry.get("unchanged"):
            continue
        ...
        for value in values:
            ...
            if vtype == "reference_image" and not value.get("ref"):
                problems.append(
                    ...
                    % (name, vid))
            if vtype == "unchanged":
                pass                      # the photograph is the specification
            elif vtype == "word":
                problems.append(
