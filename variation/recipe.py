...
META_PROMPT = """\
You are compiling a configuration file for an automated product-variation image
...
LOCK_PRODUCT = """\
The product in the output must be the identical physical object shown in the input \
image. Preserve exactly: its overall proportions and the ratio of every part to every \
other part; its silhouette and outline; the shape, curvature and edge profile of every \
component; the number of parts and how they join; material thickness; the size, shape \
and placement of any hardware, fittings or fixings; and the gauge, position and path of \
any visible cable. Do not redesign, restyle, re-proportion, simplify, embellish, \
modernise, straighten, tidy, or otherwise improve the product. Do not correct anything \
you perceive as a flaw or asymmetry. Do not add or remove any part or detail. Reproduce \
the object exactly as photographed."""

LOCK_SCENE = """\
Keep every other aspect of the image identical to the input image: camera angle, camera \
distance, focal length, framing, crop, the subject's position within the frame, the \
subject's scale within the frame, the background, the surface it rests on, lighting \
direction, lighting intensity and colour temperature, and the shape, softness and \
position of the shadow. Change only what is explicitly named above."""

DEFAULT_TEMPLATES = {
    "reference_image":
        "Change only the {region} of the product to match the material shown in the "
